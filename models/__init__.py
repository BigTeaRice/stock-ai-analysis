...
# 導入子模塊 (移除了不存在的 linear_regression)
...
# 控制導入行為 (移除了 LinearRegressionModel)
__all__ = [
    'LSTMModel',
    'ARIMAModel', 
    'create_model',
...
def create_model(model_type='lstm'):
    ...
    Args:
        model_type (str): 模型類型，可選 'lstm', 'arima'
    
    Returns:
        模型實例
    """
    # 移除了 'linear': LinearRegressionModel
    model_map = {
        'lstm': LSTMModel,
        'arima': ARIMAModel
    }
    ...
    if model_type not in model_map:
        raise ValueError(f"不支持的模型類型: {model_type}。目前僅支持 {list(model_map.keys())}")
